...
# Windows only import for reading chrome version from registry.
...
def get_chrome_major_version() -> int | None:
    '''
    Tries to detect the installed Google Chrome major version.

    Returns:
        int: Major version (e.g. 145) if detected, otherwise None.
    '''
    try:
        if sys.platform.startswith('win'):
            # Chrome stores its version in the registry at this location.
            ...
            # macOS/Linux: try common CLI invocations for Chrome/Chromium
