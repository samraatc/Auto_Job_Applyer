...
Usage:
    python -m database.migrations            # full migration
    python -m database.migrations --dry-run  # just print what it would do
...
SERVER_DIR = os.path.abspath(os.path.join(THIS, ".."))
...
from database.mongo_client import get_db, is_enabled, MONGODB_DB  # type: ignore
from repositories.store import (  # type: ignore
...
def migrate_companies(db, dry: bool) -> int:
    from config.config_manager import read_config  # type: ignore
