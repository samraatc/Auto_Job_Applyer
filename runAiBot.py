...
def apply_filters() -> None:
    ...
    try:
        ...
        pyautogui.confirm(f"Faced error while applying filters. Please make sure correct filters are selected, click on show results and click on any button of this dialog, I know it sucks. Can't turn off Pause after search when error occurs! ERROR: {e}", "Filters Error", ["Doesn't look good, but Continue XD", "Look's good, Continue"])
        # print_lg(e)
...
def _mongo_record(entry: dict, status: str) -> None:
    """Best-effort mirror of an applied/failed row into Mongo. No-op if server/ is unimportable or Mongo is down."""
    try:
        import sys as _sys, os as _os
        _sys.path.insert(0, _os.path.abspath(_os.path.join(_os.path.dirname(__file__), "server")))
        from store import record_applied  # type: ignore
        record_applied(entry, status=status)
    except Exception:
        pass  # Mongo is optional; CSV is the source of truth on failure
...
def failed_job(job_id: str, job_link: str, resume: str, date_listed, error: str, exception: Exception, application_link: str, screenshot_name: str) -> None:
    ...
    try:
        with open(failed_file_name, 'a', newline='', encoding='utf-8') as file:
            ...
            row = {'Job ID':truncate_for_csv(job_id), 'Job Link':truncate_for_csv(job_link), 'Resume Tried':truncate_for_csv(resume), 'Date listed':truncate_for_csv(date_listed), 'Date Tried':datetime.now(), 'Assumed Reason':truncate_for_csv(error), 'Stack Trace':truncate_for_csv(exception), 'External Job link':truncate_for_csv(application_link), 'Screenshot Name':truncate_for_csv(screenshot_name)}
            writer.writerow(row)
            file.close()
        _mongo_record(row, status="failed")
    except Exception as e:
...
def submitted_jobs(job_id: str, title: str, company: str, work_location: str, work_style: str, description: str, experience_required: int | Literal['Unknown', 'Error in extraction'], 
    ...
    try:
        with open(file_name, mode='a', newline='', encoding='utf-8') as csv_file:
            ...
            row = {'Job ID':truncate_for_csv(job_id), 'Title':truncate_for_csv(title), 'Company':truncate_for_csv(company), 'Work Location':truncate_for_csv(work_location), 'Work Style':truncate_for_csv(work_style),
            ...
            writer.writerow(row)
        csv_file.close()
        _mongo_record(row, status="applied")
    except Exception as e:
...
def apply_to_jobs(search_terms: list[str]) -> None:
    ...
    _base_resume_path = default_resume_path
    for searchTerm in search_terms:
        _per_term = (globals().get("per_term_resume") or {}).get(searchTerm)
        if _per_term and os.path.exists(_per_term):
            globals()["default_resume_path"] = _per_term
            print_lg(f'per_term_resume: using "{_per_term}" for "{searchTerm}"')
        else:
        ...
        try:
            while current_count < switch_number:
                ...
                for job in job_listings:
                    ...
                    uploaded = False
                    _mode = (globals().get("apply_mode") or "both").lower()
                    _has_easy = try_xp(driver, ".//button[contains(@class,'jobs-apply-button') and contains(@class, 'artdeco-button--3') and contains(@aria-label, 'Easy')]")
                    if _mode == "external" and _has_easy:
                        print_lg(f"apply_mode=external: skipping Easy Apply job '{title}'")
                        skip_count += 1
                        continue
                    if _mode == "easy" and not _has_easy:
